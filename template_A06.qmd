...
[https://github.com/Javih10/Hernandez_Javier_201_A06](https://github.com/Javih10/Hernandez_Javier_201_A06)
...
```{r}
val1 <- player_info[[2]][[4]]
print(val1)

val2 <- player_info$LJ$points
print(val2)
```
...
```{r}
height <- list(
  MJ <- (player_info$MJ$height = 78),
  LJ <- (player_info$LJ$height = 81),
  KB <- (player_info$KB$height = 78),
  KA <- (player_info$KA$height = 86),
  SO <- (player_info$SO$height = 85)
)
...
rounding <- function(int){
  if(int < 100){
    a <- round(int/10,digits = 0)*10
    return(a)   
  }else if(int < 1000){
    return(round(int/100,digits = 0) * 100)
  }else{
    return(round(int/1000, digits = 0) * 1000)
  }
}
...
new_frame <- as.data.frame(max_temps)
Colorado <- new_frame[new_frame$state == 'CO',]
Ohio <- new_frame[new_frame$state == 'OH',]
Virginia <- new_frame[new_frame$state == 'VA',]
...
for (i in 1:nrow(Colorado)){
  temps <- Colorado[i,][3:length(Colorado[i,])]
  year <- Colorado[i,]$year
  medians[as.character(year)] <-  median(as.numeric(temps))
}
max_t <- which.max(medians)

min_t <- which.min(medians)


print(sprintf("The maximum median annual temperature of %s occurred in %s and the minimum median annual temperature of %s occurred in %s.",medians[max_t],names(max_t),medians[min_t],names(min_t) ))
